...
class PrefillKV(torch.nn.Module):
 ...
 def forward(self,x):
  ...
  return tuple(y)
a=argparse.ArgumentParser();a.add_argument('--model',type=Path,required=True);a.add_argument('--audio',type=Path,required=True);a.add_argument('--output',type=Path,required=True);a.add_argument('--audio-max-seconds',type=float,default=8);a.add_argument('--audio-padding',choices=('longest','max_length'),default='max_length');z=a.parse_args();torch.set_grad_enabled(False)
p=AutoProcessor.from_pretrained(z.model,trust_remote_code=True);m=AutoModelForCausalLM.from_pretrained(z.model,trust_remote_code=True,torch_dtype=torch.float32,attn_implementation='eager').eval();c=[{'role':'user','content':[{'type':'audio','path':str(z.audio)},{'type':'text','text':P}]}];b=dict(p.apply_chat_template(c,return_tensors='pt',sampling_rate=16000,audio_padding=z.audio_padding,add_generation_prompt=True,audio_max_length=round(z.audio_max_seconds*16000),text_kwargs={'padding':'longest','truncation':True,'max_length':1000}))
