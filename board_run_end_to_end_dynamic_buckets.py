...
encoder=z.encoder_rknn or asset('encoder/audio_encoder_f800_fp16.rknn','/root/audio8-asr/encoder_f800_v2/audio_encoder_f800_fp16.rknn')
try:
 enc=load(encoder);adp=load(adapter);pre=load(z.prefill_rknn);allr=[enc,adp,pre]
 ...
 allr=[];kvs=[load(asset(f'decoder/kv/layer{i}/kv_fp16.rknn',f'/root/audio8-asr/decoder_3d_fixed_s128/layer{i}/kv_fp16.rknn')) for i in range(8)];heads=[load(asset(f'decoder/head_shards/shard{i:02d}/head_fp16.rknn',f'/root/audio8-asr/decoder_3d_fixed_s128/head_shards/shard{i:02d}/head_fp16.rknn')) for i in range(8)];allr=kvs+heads;blocks={}
 def get_blocks(c):
  if c not in blocks:
  ...
  return blocks[c]
 initial=po[1].shape[-2]
 capacity=next((c for c in (128,256,512) if initial<c),None)
 if capacity is None: raise RuntimeError(f'prefill length {initial} needs an unbuilt bucket above 512')
 ck=[];cv=[]
 for i in range(8):
